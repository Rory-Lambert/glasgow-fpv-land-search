...
# 2. Generate the satellite view (Esri). The housing-proximity image needs
#    Overpass — skip gracefully if it's unavailable (the cached housing figure
#    still appears in the issue body).
IMG_REL=$(python3 analysis/fetch_satellite.py --code "$CODE" | tail -n1)
PROX_REL=$(python3 analysis/site_proximity.py --code "$CODE" 2>/dev/null | tail -n1 || true)
[ -n "${PROX_REL:-}" ] && [ ! -f "$PROX_REL" ] && PROX_REL=""
echo "Images: $IMG_REL ${PROX_REL:+and $PROX_REL}"

# 3. Commit & push so the raw URLs resolve before the issue references them.
git add "$IMG_REL" ${PROX_REL:+"$PROX_REL"}
git commit -q -m "Add site views for $CODE ($NAME)"
git push -q
